-- Persist the per-invoice seller, impose canonical numeric batch order, and
-- make legitimate retries safe while the monthly advisory lock is held.
...
declare
  ...
  customer_id uuid; seller_label text; stored_seller text;
begin
  ...
  -- Validate the complete batch before writing any row. Seller labels must be
  -- the exact canonical catalog value for this organization.
  for item in select value from jsonb_array_elements(p_invoices) loop
    ...
    if invoice_ref = '' or invoice_ref <> coalesce(item->>'nro_fact','')
    ...
    if exists(select 1 from public.facturas_maestras f
      where f.ref_fact=invoice_ref and f.organization_id is distinct from org) then
      raise exception 'invoice % belongs to another organization',invoice_ref;
    end if;
    select r.vendedor into stored_seller from public.reportes_ventas r
      where r.organization_id=org and r.mes_reporte=report_name
        and r.ref_fact=invoice_ref;
    if found and trim(coalesce(stored_seller,'')) not in ('',seller_label) then
      raise exception 'invoice % already has a different seller',invoice_ref;
    end if;
  end loop;
  ...
  for item in
    select value from jsonb_array_elements(p_invoices)
    order by case when trim(value->>'ref_fact') ~ '^[0-9]+$' then 0 else 1 end,
      ...
      trim(value->>'ref_fact')
  loop
    invoice_ref := trim(item->>'ref_fact');
    seller_label := trim(item->>'vendedor');
    invoice_date := (item->>'fecha')::date;

    if not exists(select 1 from public.facturas_maestras f
      where f.ref_fact=invoice_ref and f.organization_id=org) then
    ...
    if exists(select 1 from public.reportes_ventas r
      ...
      update public.reportes_ventas r set vendedor=seller_label
        where r.organization_id=org and r.mes_reporte=report_name
          and r.ref_fact=invoice_ref and trim(coalesce(r.vendedor,''))='';
    else
      next_row := next_row + 1;
