-- Receipt numbers are optional. Keep the report reference and the actual
-- invoice number as separate values when a row is saved.
drop function if exists public.enterprise_save_report_row(
...
begin
  ...
  if exists (
    select 1 from jsonb_array_elements(p_payments) value
  ...
  if nullif(trim(p_ref_fact),'') is not null then
    if exists(select 1 from public.facturas_maestras
              where ref_fact=trim(p_ref_fact)
                and organization_id is distinct from org) then
    ...
    insert into public.facturas_maestras(
      ...
      org,trim(p_ref_fact),trim(p_cliente),trim(coalesce(p_commercial_name,'')),
      p_invoice_date,coalesce(nullif(trim(p_invoice_number),''),trim(p_ref_fact)),p_sale
    )
    ...
    insert into public.billing_customers(
      ...
      org,trim(p_cliente),trim(coalesce(p_commercial_name,'')),auth.uid()
    )
    on conflict(organization_id,normalized_name,normalized_commercial_name)
    do update set updated_at=clock_timestamp(),updated_by=auth.uid()
    ...
    on conflict(organization_id,factura_id) do update set
      customer_id=excluded.customer_id,updated_at=clock_timestamp(),
      updated_by=auth.uid();
    perform public.sync_enterprise_reminder(
      org,trim(p_ref_fact),p_request_id,
      'Reprogramación automática por edición de factura'
    );
  end if;

  if exists(select 1 from public.reportes_ventas
            where nro_fila=p_row_number and mes_reporte=p_report_name
              and organization_id is distinct from org) then
    raise exception 'report row belongs to another organization';
  end if;
  insert into public.reportes_ventas(
    ...
    org,p_row_number,trim(p_ref_fact),p_seller,p_nail_polish,p_payments,
    receipts,p_payment_comments,p_report_name
  )
  on conflict(nro_fila,mes_reporte) do update set
    ref_fact=excluded.ref_fact,vendedor=excluded.vendedor,
    esmaltes=excluded.esmaltes,abonos=excluded.abonos,
    numeros_recibo=excluded.numeros_recibo,
  ...
  values(org,p_request_id,'save_report_row','{}',clock_timestamp());
end; $$;
