                                ...
                                LayoutBuilder(
                                    builder: (context, k) => GridView.count(
                                            ...
                                            children: [
                                              ...
                                              _Kpi('FACTURAS', '${r.facturas}',
                                                  Icons.receipt_long_outlined),
                                              _Kpi('CLIENTES', '${r.clientes}',
...
class _Kpi extends StatelessWidget {
  const _Kpi(this.titulo, this.valor, this.icono,
      {this.detalle, this.positivo = false});
          ...
          child: Row(children: [
            Container(
                ...
                decoration: BoxDecoration(
                    color: c.hg.hover, borderRadius: BorderRadius.circular(10)),
...
class _Mayor extends StatelessWidget {
  ...
  Widget build(BuildContext c) {
    final r = registro;
    return _Kpi('MAYOR VENTA', r == null ? '—' : dinero(r.venta),
                ...
                children: [
                  Text(titulo,
                      style: Theme.of(c).textTheme.titleMedium?.copyWith(
                          fontWeight: FontWeight.w700, color: c.hg.plum)),
...
class _Cobranza extends StatelessWidget {
  ...
  Widget build(BuildContext c) {
    ...
    return Column(children: [
      ...
      Text('${r.porcentajeCobranza.toStringAsFixed(1)}% de cobranza',
          ...
          rows: datos
              .map((v) => DataRow(cells: [
                    DataCell(SizedBox(
                        width: 240,
                        child:
                            Text(v.nombre, overflow: TextOverflow.ellipsis))),
                    DataCell(Text('${v.facturas}')),
...
class _LineChartState extends State<_LineChart> {
  ...
  void _seleccionar(double x, double ancho) {
    ...
    final i = widget.puntos.length == 1
        ? 0
        : ((x - 12) / (ancho - 24) * (widget.puntos.length - 1))
            .round()
            .clamp(0, widget.puntos.length - 1);
  ...
  Widget build(BuildContext c) {
    if (widget.puntos.isEmpty) {
      return const _Estado(
          icon: Icons.show_chart, texto: 'No hay datos para graficar.');
    }
    return Column(children: [
      ...
      Expanded(child: LayoutBuilder(builder: (c, k) {
        ...
        return MouseRegion(
            ...
            child: GestureDetector(
                ...
                onTapDown: (e) => _seleccionar(e.localPosition.dx, k.maxWidth),
  ...
  void paint(Canvas c, Size s) {
    ...
    for (var i = 0; i < p.length; i += step) {
      final tp = TextPainter(
          text: TextSpan(
              text: p[i].etiqueta, style: TextStyle(fontSize: 9, color: text)),
      ...
      tp.paint(
          c,
          Offset((x - tp.width / 2).clamp(0, s.width - tp.width),
              s.height - bot + 8));
    }
  }
...
class _BarChart extends StatelessWidget {
  ...
  final String Function(double) dinero;
  @override
  Widget build(BuildContext c) => LayoutBuilder(builder: (c, k) {
        final maxV = math.max(
            1, datos.fold<double>(0, (m, e) => math.max(m, e.promedio)));
        return Row(
                            ...
                            child: Column(
                                ...
                                children: [
                                  Text(
                                      ...
                                      maxLines: 1,
                                      style: TextStyle(
                                          fontSize: 8, color: c.hg.mutedText)),
                                  const SizedBox(height: 3),
                                  Container(
                                      height: (150 * e.promedio / maxV)
                                          .clamp(2, 150),
                                      decoration: BoxDecoration(
                                          color: c.hg.lilac,
                                          borderRadius:
